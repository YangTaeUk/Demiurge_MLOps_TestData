...
> 9단계 마일스톤 — Foundation부터 데이터 제공 API까지
...
**추가 의존성**: `pyarrow`, `fastavro`, `lxml` (formats 그룹), `orjson`, `msgspec`, `cramjam` (handlers 그룹)
...
### Phase 8: 데이터 수집 및 시딩 (Weeks 10-11)

**목표**: Kaggle 다운로드 모듈, 데이터셋 매니페스트, 구조화 시딩 파이프라인

**산출물**:

| 카테고리 | 파일 | 설명 |
|---------|------|------|
| **데이터** | `data/` 디렉토리 | `.gitkeep` + `.gitignore` (원본 데이터 미포함) |
| **다운로더** | `data/downloader.py` | KaggleDownloader 클래스 (kaggle CLI 래핑) |
| **매니페스트** | `configs/datasets/manifest.yaml` | 32종 데이터셋 메타데이터 (kaggle_id, 파일, 시딩 대상) |
| **시딩** | `core/seed.py` | SeedPipeline 클래스 (카테고리별 시딩 전략) |
| **검증** | `data/validator.py` | DataValidator 클래스 (CSV 무결성 검증) |
| **CLI** | `__main__.py` 확장 | `download`, `seed`, `setup` 명령어 추가 |
| **Makefile** | `Makefile` 확장 | `download`, `seed`, `setup-data` 타겟 추가 |
| **테스트** | `tests/unit/test_seed.py` | SeedPipeline 단위 테스트 |
| **테스트** | `tests/integration/test_seed_*.py` | 카테고리별 시딩 통합 테스트 |

**추가 의존성**: `kaggle`, `tqdm`

**완료 기준**:
- `python -m demiurge_testdata download --dataset home_credit` → `data/raw/home_credit/*.csv` 다운로드
- `python -m demiurge_testdata seed --dataset home_credit` → PostgreSQL에 `home_credit` 테이블 생성 + 레코드 삽입
- `psql -h localhost -p 5434 -U testdata -d testdata -c "SELECT COUNT(*) FROM home_credit"` → 307,511
- `SELECT sk_id_curr, target, amt_income_total FROM home_credit WHERE target=1 LIMIT 10` → 구조화된 결과 반환
- SeedPipeline이 4개 카테고리 (RDBMS, NoSQL, Storage, Streaming) 모두 동작
- manifest.yaml의 32종 데이터셋 정의 완성

---

### Phase 9: 데이터 제공 API 확장 (Weeks 11-12)

**목표**: FetchPipeline, REST 데이터 조회 API, gRPC FetchData/StreamData RPC

**산출물**:

| 카테고리 | 파일 | 설명 |
|---------|------|------|
| **파이프라인** | `core/fetch.py` | FetchPipeline 클래스 (Adapter.fetch() → HandlerChain.decode()) |
| **REST** | `api/rest/app.py` 확장 | `/data/datasets`, `/data/{adapter}/query`, `/generate/{dataset}` 엔드포인트 |
| **gRPC** | `protos/testdata/v1/data.proto` | FetchData, StreamData, ListDatasets RPC + Protobuf 메시지 |
| **gRPC** | `api/grpc/server.py` 확장 | DataServiceServicer 구현 |
| **테스트** | `tests/unit/test_fetch_pipeline.py` | FetchPipeline 단위 테스트 |
| **테스트** | `tests/integration/test_data_api.py` | REST/gRPC 데이터 조회 통합 테스트 |

**완료 기준**:
- `GET /data/datasets` → 시딩된 데이터셋 목록 반환
- `GET /data/postgresql/query?table=home_credit&where=target%3D1&limit=10` → 구조화된 데이터 반환
- `GET /data/datasets/home_credit/sample?limit=100` → 샘플 데이터 반환
- `GET /generate/home_credit?limit=100` → 온디맨드 생성 데이터 반환
- gRPC `FetchData` RPC → 단건 조회 동작
- gRPC `StreamData` RPC → 스트리밍 조회 동작
- FetchPipeline이 RDBMS, NoSQL, Storage 3종에서 동작
...
Phase 8 (데이터 수집 및 시딩)
  │  Kaggle 다운로드 + 구조화 시딩.
  │  push(bytes) 외에 실제 테이블/컬렉션 생성.
  │  외부 시스템이 네이티브 쿼리로 데이터 소비 가능.
  ▼
Phase 9 (데이터 제공 API 확장)
  │  FetchPipeline으로 시딩 데이터 API 제공.
  │  REST/gRPC로 데이터 조회 엔드포인트 추가.
```
...
graph LR
    ...
    P8["Phase 8<br/>데이터 수집 + 시딩<br/>Weeks 10-11"]
    P9["Phase 9<br/>데이터 제공 API<br/>Weeks 11-12"]
...
all = ["demiurge-testdata[handlers,formats,rdbms,nosql,streaming,storage,filetransfer,api]"]
...
| [11-데이터-수집-시딩](./11-데이터-수집-시딩.md) | Phase 8의 상세 설계 |
| [12-데이터-제공-설계](./12-데이터-제공-설계.md) | Phase 9의 상세 설계 |
