...
`BaseAdapter.fetch(query: dict, limit: int)` 메서드는 카테고리별로 다른 query dict 구조를 사용한다. 시딩된 구조화 데이터를 조회하거나, push()로 저장된 바이트를 역직렬화하는 데 사용한다.

### 11.2 카테고리별 query dict 정의

#### RDBMS

```python
query = {
    "table": "home_credit",                    # 필수: 테이블명
    "where": "target = 1",                     # 선택: WHERE 절 (문자열)
    "columns": ["sk_id_curr", "amt_income_total"],  # 선택: SELECT 컬럼 (기본: *)
    "order_by": "sk_id_curr ASC",              # 선택: ORDER BY
    "offset": 0,                               # 선택: OFFSET (페이지네이션)
}
# → SELECT columns FROM table WHERE where ORDER BY order_by LIMIT limit OFFSET offset
```
...
```python
query = {
    "collection": "instacart_orders",          # 필수: 컬렉션명
    "filter": {"order_dow": 0},                # 선택: MongoDB 필터 (dict)
    "projection": {"order_id": 1, "user_id": 1},  # 선택: 반환 필드
    "sort": [("order_number", -1)],            # 선택: 정렬 기준
}
# MongoDB: collection.find(filter, projection).sort(sort).limit(limit)
# Elasticsearch: index.search(query=filter, _source=projection, size=limit)
# Redis: SCAN match=collection:* COUNT limit
```
...
```python
query = {
    "key": "hm/transactions_train.parquet",     # 선택: 특정 객체 키
    "prefix": "hm/",                           # 선택: 키 프리픽스 (목록 조회)
}
# key 지정: adapter.read(key) → bytes (단일 파일 읽기)
# prefix 지정: adapter.list_keys(prefix) → [key, ...] (파일 목록)
```
...
```python
query = {
    "topic": "ieee-fraud-events",              # 필수: 토픽명
    "group_id": "mlops-consumer",              # 선택: Consumer 그룹
    "offset": "earliest",                      # 선택: "earliest" | "latest"
}
# adapter.subscribe(topic, callback) 기반
# 주의: Streaming fetch()는 이 프로젝트에서 stub — 외부 시스템이 직접 소비
```
...
```python
query = {
    "remote_path": "/data/home_credit.csv",    # 선택: 특정 파일 경로
    "remote_dir": "/data/",                    # 선택: 디렉토리 목록 조회
}
# remote_path: adapter.download(remote_path, local_path)
# remote_dir: adapter.list_files(remote_dir)
```
...
시딩은 push(bytes)와 다른 경로를 사용한다. 어댑터 카테고리별 확장 메서드를 통해 **구조화된 데이터**를 직접 적재한다. 이 메서드들은 이미 Category Base에 정의되어 있으며, SeedPipeline이 이를 호출한다.
...
class BaseRDBMSAdapter(BaseAdapter):
    ...
    async def create_table(self, name: str, columns: list[dict]) -> None:
        """
        DDL 실행으로 테이블 생성.

        Args:
            name: 테이블명 (예: "home_credit")
            columns: [{"name": "sk_id_curr", "type": "BIGINT"}, ...]

        시딩 사용:
            스키마 추론기가 CSV 컬럼에서 columns를 자동 생성
        """
    ...
    async def bulk_insert(self, table: str, records: list[dict]) -> int:
        ...
        Args:
            table: 대상 테이블명
            records: [{"sk_id_curr": 100000, "amt_income_total": 50000.0, ...}, ...]

        시딩 사용:
            CSV → list[dict] → batch_size 단위로 bulk_insert 반복
            PostgreSQL: asyncpg.copy_records_to_table() 활용
        """
```
...
class BaseNoSQLAdapter(BaseAdapter):
    ...
    async def insert_documents(self, collection: str, documents: list[dict]) -> int:
        """
        다건 문서 삽입.

        Args:
            collection: 컬렉션명 (예: "instacart_orders")
            documents: [{order_id: 1, ...}, {order_id: 2, ...}, ...]

        시딩 사용:
            CSV → list[dict] → batch_size 단위로 insert_documents 반복
            MongoDB: collection.insert_many(documents)
            Elasticsearch: helpers.async_bulk(documents)
        """
```
...
class BaseStorageAdapter(BaseAdapter):
    # 기존 메서드 — 시딩에 HandlerChain과 함께 사용
    ...
    async def write(self, key: str, data: bytes) -> None:
        ...
        시딩 사용:
            CSV → list[dict] → HandlerChain.encode() (Parquet 등) → write(key, bytes)
            결과: S3에 구조화된 Parquet 파일 생성
        """
```

### 12.5 Streaming 시딩 메서드
...
class BaseStreamAdapter(BaseAdapter):
    ...
    async def publish(self, topic: str, message: bytes) -> None:
        ...
        시딩 사용:
            CSV → list[dict] → HandlerChain.encode() (JSON) → publish(topic, bytes)
            결과: Kafka 토픽에 구조화된 JSON 메시지 발행
        """
```

### 12.6 시딩 vs push 메서드 비교

| 작업 | push(bytes) 경로 | 시딩 경로 |
|------|-----------------|---------|
| **RDBMS** | `push(bytes)` → `INSERT INTO raw_data` | `create_table()` → `bulk_insert(records)` |
| **NoSQL** | `push(bytes)` → `{data: Binary(bytes)}` | `insert_documents(collection, [dict])` |
| **Storage** | `push(bytes)` → `write(key, bytes)` | `encode(records)` → `write(key, bytes)` |
| **Streaming** | `push(bytes)` → `publish(topic, bytes)` | `encode(records)` → `publish(topic, bytes)` |
...
| [02-데이터-흐름](./02-데이터-흐름.md) | push(bytes)·시딩·제공 흐름 비교 |
...
| [11-데이터-수집-시딩](./11-데이터-수집-시딩.md) | SeedPipeline 상세 설계 |
| [12-데이터-제공-설계](./12-데이터-제공-설계.md) | fetch() 기반 데이터 제공 |
